# HaMakom

HaMakom is a mobile-first bilingual React/Vite app for observant Jewish singles in Israel. The product direction is planner-first: one strong date plan first, calm alternatives second.

## Stack

- React + Vite
- Supabase Auth / Database / Storage
- Leaflet / React Leaflet
- Vercel deployment

## Local Development

1. Install dependencies:

```bash
npm install
```

2. Copy `.env.example` to `.env.local` and set real values:

```bash
VITE_SUPABASE_URL=...
VITE_SUPABASE_ANON_KEY=...
VITE_ADMIN_PIN=...
```

3. Start the app:

```bash
npm run dev
```

## Pre-Deploy Checklist

Blocking:
- Set production `VITE_SUPABASE_URL`
- Set production `VITE_SUPABASE_ANON_KEY`
- Change `VITE_ADMIN_PIN` from the default `1234`
- Run the SQL from the Admin `SQL` tab in Supabase
- Make sure Google auth and email magic links are enabled in Supabase Auth
- Add your production app URL to Supabase Auth redirect URLs
- Create the `location-images` storage bucket in Supabase if image upload is needed

Quality checks:
- `npm run build`
- `npm run lint`
- Test quiz -> result -> save gate -> sign in -> restore flow
- Test saved plans and saved places
- Test admin login
- Test analytics tab after real events exist
- Test Hebrew / RTL screens on mobile

## Supabase Setup

Required features used by the app:
- Auth
- Database tables for locations, pending submissions, quiz results, analytics, feedback
- Storage bucket: `location-images`

The Admin `SQL` tab contains the first-pass schema for:
- `analytics_events`
- `user_feedback`
- `recommendation_impressions`
- `recommendation_outcomes`

## Adding A New Location (Checklist)

When adding a place, update all relevant layers so it appears consistently in Explore, quiz flows, and production:

1. Add the location in `src/data/locations.js` with the same shape as existing entries:
   - `id`, `name`, `name_he`, `city`, `city_he`
   - `category`, `occasion`, `price`, `date_stage`
   - `description`, `description_he`, `maps_query`
   - `kashrus`, `featured`, `status` (usually `approved`)
2. If the city is new, add it to `src/lib/constants.js`:
   - Include city in `CITIES`
   - Add city coordinates to `CITY_COORDS` for map support
3. If relevant, add/update a plan in `src/data/datePlans.js` that uses the place in `stops`.
4. Keep quiz city text complete by adding any new plan city copy in `src/lib/quiz.js` (`cityText` map).
5. If Supabase is enabled, insert the same location into the `locations` table with `status='approved'`.
   - The app will prefer DB rows over seed fallback when Supabase data is available.
6. Verify in UI:
   - Explore search finds both English and Hebrew name/city variants
   - City/category filters include and show the place
   - Map pin appears when city coordinates exist

## Vercel Deployment

1. Import the repo into Vercel
2. Framework preset: `Vite`
3. Add environment variables:
   - `VITE_SUPABASE_URL`
   - `VITE_SUPABASE_ANON_KEY`
   - `VITE_ADMIN_PIN`
   - `VITE_PUBLIC_APP_URL`
4. Deploy
5. In Supabase Auth, add the Vercel production URL and preview URL pattern as redirect URLs

The repo already includes [vercel.json](vercel.json) with SPA rewrites back to `index.html`.

## Mobile Apps (iOS + Android)

The same web build is wrapped as native apps with [Capacitor](https://capacitorjs.com/).
The native projects live in `android/` and `ios/`. App identity:

- **App ID / bundle:** `app.hamakom`
- **App name:** `HaMakom`
- **Deep-link scheme:** `app.hamakom://auth-callback` (used to return from sign-in)

### Tooling prerequisites

- **Android:** [Android Studio](https://developer.android.com/studio) (includes the SDK + an emulator). Works on Windows/macOS/Linux.
- **iOS:** a **Mac** with Xcode. iOS apps cannot be built on Windows — use a Mac, or a cloud-Mac CI service (Codemagic, Ionic Appflow, or a macOS GitHub Actions runner).
- Accounts: **Google Play Developer** ($25 one-time) and **Apple Developer** ($99/yr).

### Day-to-day workflow

Any time you change web code, rebuild and copy it into the native shells:

```bash
npm run cap:sync          # build web + copy into android & ios
npm run android           # build + open Android Studio
npm run ios               # build + open Xcode (Mac only)
npm run cap:assets        # regenerate icons/splash after logo changes
```

Icons/splash are generated from `assets/icon.png` + `assets/splash.png`, which
are produced by `node scripts/generate-native-assets.mjs` from the traced logo.

### One-time Supabase config (required for sign-in inside the app)

In the Supabase dashboard → **Authentication → URL Configuration → Redirect URLs**, add:

```
app.hamakom://auth-callback
```

Without this, Google login and email magic links will not return into the app.
Email magic links open in the system browser and then redirect back via the
custom scheme. (Universal Links / App Links are a nicer future upgrade but
require hosting `assetlinks.json` and `apple-app-site-association` on hamakom.app.)

Environment variables (`VITE_SUPABASE_URL`, etc.) are baked into the web build
at `npm run build` time, so set production values before `cap:sync`.

### Ship Android (Play Store)

1. `npm run android` to open Android Studio.
2. Set a signing key: **Build → Generate Signed Bundle / APK → Android App Bundle**, create/keep a keystore (back it up — losing it blocks future updates).
3. Bump version in `android/app/build.gradle` (`versionCode` + `versionName`) for each release.
4. Produce a signed **.aab** and upload it in the [Play Console](https://play.google.com/console) → create app → Production (or Internal testing first).
5. Fill store listing, content rating, data-safety form, privacy policy URL (`https://hamakom.app` privacy page), then submit for review.

### Ship iOS (App Store) — Mac required

1. On a Mac: `npm run cap:sync` then `npx cap open ios`.
2. In Xcode, select the **App** target → **Signing & Capabilities** → pick your Apple Developer team (auto-manages the provisioning profile).
3. Set version/build under **General**.
4. **Product → Archive**, then **Distribute App → App Store Connect**.
5. In [App Store Connect](https://appstoreconnect.apple.com): complete the listing, screenshots, privacy questionnaire, then submit for review.

### Notes / current native gaps

- Geolocation permission strings are set (Android manifest + iOS `Info.plist`); the app uses the browser geolocation API inside the webview.
- Reminders are still local-only (no native push notifications yet).
- The `android/` and `ios/` folders are committed, but their build outputs and the copied `public/` web assets are gitignored and regenerated by `cap sync`.

## Current Known Gaps

- Reminders are local-only, not push notifications or calendar events
- Date plan admin edits are local-browser storage based, not multi-admin cloud persistence
- Analytics dashboard is client-side aggregated from Supabase rows; SQL views would be better at scale
- Recommendation learning is still heuristic plus event capture, not a full adaptive model
