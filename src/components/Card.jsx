...
export default function Card({ loc, lang, tx, saved, onToggleSave, onClick, showSave = true }) {
  ...
  return <article className={`ui-place-card ${marketOf(loc).id === 'ny' && !loc.image_url ? 'ui-place-card-text' : ''}`}>
    <a href={locationPath(loc)} onClick={e => { if (onClick) { e.preventDefault(); onClick() } }}>
      <div className="ui-place-image">{loc.image_url && !failed ? <img src={loc.image_url} alt="" onError={() => setFailed(true)} loading="lazy" /> : <Icon name={isFoodVenue(loc) ? 'menu' : /Park/.test(loc.category) ? 'map' : 'sparkle'} size={32} />}{loc.is_partner && <span className="ui-partner-badge">{tx.partnerBadge}</span>}</div>
