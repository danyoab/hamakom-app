...
export default function MapView({
  ...
  const mapAreaStyle = embedded
    ? {
        position: 'absolute',
        inset: 0,
        bottom: selectedCity ? panelHeight : 0,
      }
    : {
        position: 'fixed',
        top: topOffset,
        ...
        <MapContainer center={[31.8, 35.0]} zoom={8} style={{ width: '100%', height: '100%' }} zoomControl>
          ...
          {Object.entries(cityGroups).map(([city, cityLocations]) => (
            <Marker
              ...
              icon={cityIcon(cityLocations.length, selectedCity === city)}
        ...
        <div style={cityPanelStyle}>
          ...
          <div style={{ overflowY: 'auto', flex: 1, padding: '0 10px 12px' }}>
            {panelLocations.map((location) => {
              const name = lang === 'he' ? location.name_he || location.name : location.name
